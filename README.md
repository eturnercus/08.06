...
| Направление | Статус |
|-------------|--------|
| Полноценный Chrome/WebView с DOM-кликами (как в IDE-агентах) | **Реализовано** (Live WebView + `eval` DOM-клики; preview iframe + postMessage) |
| Нативный захват экрана / OCR / STT | **Реализовано** (scrot/grim/ffmpeg; tesseract OCR; whisper STT при наличии в PATH) |
...
**Агент-браузер:** режим Preview загружает HTML через сеть в sandboxed `srcdoc` (клики по ссылкам через postMessage). Режим **Live WebView** открывает отдельное окно Tauri WebView2/WebKit с DOM-кликами (`elementFromPoint`, CSS-селектор). Сложные JS-сайты лучше смотреть в Live WebView; cross-origin ограничения CDP Chrome как в IDE-агентах пока не покрыты.

**Захват устройств:** снимок экрана через системные утилиты; OCR — `tesseract`; STT — `whisper`/`whisper-cli` при установленных моделях.
